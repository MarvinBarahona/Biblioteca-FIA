...
for (var i = 0; i < majors.length; i++) {
  ...
  if (majorName!=null) {error.push(majorName)}
  if (majorCode!=null) {error.push(majorCode)}
  ...
  for (var j = 0; j < courses.length; j++) {
    ...
    if (courseName!=null) {error.push(courseName)}
    if (courseCode!=null) {error.push(courseCode)}
  }
}
...
export default {massCreateValidation}
